...
* **F2-DEPLOY-RERECORD** — ``scripts/deploy.sh`` gains a fixture
  ...
  fails the deploy (exit >=10) on any drift in the committed contract
  fixtures. The diff EXCLUDES ``_manifest.json`` because its
  ``elapsed_s`` / ``size_bytes`` fields are non-deterministic and would
  false-fail every deploy. ``--skip-heavy`` + a ``timeout`` backstop
  keep the gate from hanging the deploy on a multi-minute corpus scan.
  This is layer 4 of the stale-fixture defence documented in
...
def test_rerecord_gate_exit_codes_are_loud():
    """Gate failures exit >=10 (same 'loud failure' band as verify /
    probe-gate / rollback). Pin the two codes the gate uses: 13 (recorder
    run failed / timed out) and 14 (fixture drift)."""
    code = _strip_bash_comments(DEPLOY_SH.read_text(encoding="utf-8"))
    idx = code.find("scripts.v2.contracts.record_fixtures")
    block = code[idx: idx + 1200]
    assert "exit 13" in block, (
        "re-record gate must `exit 13` when the recorder run itself fails "
        "(container error / timeout) (S-B7)"
    )
    assert "exit 14" in block, (
        "re-record gate must `exit 14` on fixture drift (S-B7)"
    )
