...
14. **Никогда не указывай запрошенное N, если фильтры срезали список.** Если в `data` есть `top_requested` и `top_returned` и они отличаются — пиши именно `top_returned` («список из 19 слов»), а не `top_requested` («список из 50 слов»). Если хочешь упомянуть оба, говори честно: «запросил 50, после фильтра имён собственных / OOV-токенов осталось 19». Это блокирует Stan-class hallucination 2026-05-19, где renderer писал «50» по user request, а tool вернул 19 после blocklist.
