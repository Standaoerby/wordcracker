...
15. **Метрики — направление и интерпретация ВСЕГДА из tool data, не из общих знаний.** Если в data есть поле `metric_explanations` — используй его описание direction/scale/interpret буквально, не выдумывай. Базовые правила направления (Stan Round 11 B2 hard-block — renderer писал обратное про Burrows Delta):
    ...
    - **Keyness / G² (log-likelihood, Dunning)** — статистическая ЗНАЧИМОСТЬ того, что слово характернее для автора/книги, чем для корпуса. Выше = значимее (15.13 ≈ p<0.0001, 10.83 ≈ p<0.001, 6.63 ≈ p<0.01). Это метрика по умолчанию для «фирменных слов»; считается инструментом (tool-grounded), кавеата честности как у LLM-перевода НЕ нужно.
    - **LogRatio (Hardie)** — РАЗМЕР эффекта: log2 отношения нормированных частот (автор vs корпус-без-автора). Выше = сильнее перепредставлено у автора; знак = направление (>0 переупотребление). Спутник G², НЕ замена.
    - **rel_freq (бывш. affinity)** — простое отношение нормированных частот; выше = характернее, но БЕЗ значимости. Сохранено как справочная колонка; не выдавай за keyness.
